...
> (33 万行, 全部 UTF-8, 其中 410 份带 metadata、1666 行逐字),
> 解析异常 **0**、`validate()` 问题 **0**、往返不变量违例 **0**;
> 相邻时间标签"相等" 1917 次 (改为 `debug`, 不再刷 warning)、"递减" 0 次、
> 行首重复标签 0 次. 另外, 完全为空的参考行 **0** 条、value 含方括号的 metadata
> **0** 条 —— 这两处修复对这份语料是纯防御性/纯新增能力.
> 明细见 [`docs/research.md`](docs/research.md) 的"真实语料基线".
>
> 按 SPL 对照的那一轮 (见下) 另跑了一遍**全量 6622 份 .lrc** (58.6 万非空行):
> 空正文行 28836 条, 其中 420 处 (138 个文件) 会把紧随其后、时间戳相同的真实歌词
> 挤成"翻译" —— 这一轮全部修掉, 28436 行拿到真实 `end`; 改后解析异常 0、
> `validate()` 问题 0、`dumps` 不收敛 0.

### Fixed

- **空正文行按 SPL 读作"纯结束标记", 不再产生空歌词行** (真 bug, 语料里 420 处).
  [SPL](https://moriafly.com/standards/spl.html) 明文规定"时间戳后不接任何文本内容
  的行是纯粹的结束标记: 它不会产生一句新歌词, 也不参与翻译识别", 而且这种标记
  **常与下一句歌词使用相同的时间戳**. 此前实现为它造了一条空正文行, 于是:
...
- **越界逐字标记按 SPL 忽略, 行标签/行尾标签为准** (取代 0.4.0 早期审查里的
  "按首个词元归位"). SPL 规定逐字标记必须递增且落在行开始与结束时间之间, 否则
  该标记被忽略. 于是:
...
- **序列化器不再写出"没有正文的参考行"**: 只有逐字标签、没有正文的参考行写成
  孤立行后, 解析端按 SPL 会把它读成上一行的结束标记 (既丢参考行又改上一行的
  时间范围), 因此改为跳过并记 `debug`.

### Changed

- **"多个行首标签 + 正文还有标签"的歧义用 SPL 的递增规则判定**.
  SPL 要求逐字标记递增, 于是:
  - 整行标签**非递减** → 按"行标签 + 首字标签"的逐字行读成一行 (首字延迟);
  - 出现**递减** → 按重复行简写展开, 第二行里越界的逐字标记按 SPL 忽略.
  依据: 真实语料里 199 行这种写法有 197 行是非递减的 (两个标签相差 0.1-1.3 秒),
  按重复行读会得到两条几乎相同的行. 此前一律按单行读, 会丢掉真实重复行的第二次
...
- **空正文行改用尖括号写出**. `dumps()` 遇到没有正文的行时写
  ...
  因此成为严格不动点 (2 万份随机语料 + 6622 份真实文件里首轮即定点).
...
- 新增 [SPL 对照](docs/research.md#spl-对照2026-09-19-修订版): 逐条比对结果、
  6 处有意偏离的理由与语料实测值 (标准 2026-09-19 修订新增的"延迟逐字标记"
  本库早已支持, 语料里 296 行在用).
- `README.md` 新增"与 SPL 的一致性"一节 (完全一致的部分 / 有意偏离的部分),
  并更新"往返保真度"表 (空正文行改为按 SPL 语义处理, 不再是有损场景).
- `docs/design.md` 的"待决策 / 已知取舍"改成"现象 → 为什么绕 → 当前选择 →
  若要改需要动什么"四段式, 便于下次决策时不用重新推一遍; 其中"空正文行的语义"
  与"没有正文的参考行"两条已按 SPL 收敛.

...
- 标签之后的空白算正文（`[00:01.000]` 后跟两个空格时，正文是 `"  "`），而没有标签的空白行
  仍是分隔符（重置参考行锚点）。
...
当前 `parse_lrc()` 内部已经承载了多种解析状态：metadata、行池、参考行锚点、warning
策略、歧义处理等。随着 strict mode、行号错误、source map、unsynced line 收集等功能
加入，继续平摊在单个函数里会越来越难维护。

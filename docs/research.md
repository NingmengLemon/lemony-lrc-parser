...
## SPL 对照（2026-09-19 修订版）

[SPL](https://moriafly.com/standards/spl.html)（Salt Player Lyrics，制定 2024-12-16，
最近修订 2026-09-19）是目前唯一把"LRC 家族多年踩到的兼容性问题"写成条文的文档，
所以本库把它当 LRC 语义的参照物。逐条对照的用例在
[`tests/test_spl_conformance.py`](../tests/test_spl_conformance.py)。

这一轮修订新增的是"延迟逐字标记"：`[05:20.22]<05:21.22>你好[05:24.22]`，含义是
"歌词行已到达、但首字还没开始唱"（标准注明将在 SPW 1.8 / Salt Player 11.0.0 支持）。
本库在此之前就支持这种形状（语料里 296 行在用），无需改动。

对照结果：时间戳数字规范、显式/隐式行尾、重复行简写、翻译的三种识别方式、两种逐字
括号写法、越界逐字标记忽略，这些**全部一致**；差异集中在下面几处。

### 空正文行的语义（本轮修复）

标准原文："时间戳后不接任何文本内容的行是**纯粹的结束标记**：它不会产生一句新歌词，
也不参与翻译识别……结束标记行常与下一句歌词使用相同的时间戳"。

改动前的实现为这种行造一条**空正文占位行**，于是紧随其后、时间戳相同的真实歌词行
被挂成它的 `reference_lines`（翻译），`line.text` 是空串 —— 按行遍历的消费者整行丢失。
对 6622 份文件（58.6 万非空行）的统计：

| 指标 | 数值 |
| --- | --- |
| 空正文行（方括号标签之后完全为空） | 28836 |
| 其中能找到"开始时间更早的上一行"（可当行尾） | 28279 |
| 其中下一非空行首标签相同 | 1458 |
| **空行吞掉下一句真实歌词（138 个文件）** | **420** |
| 标签之后只剩空白的行 | 0 |
| 一行上有多个标签的空行 | 0 |

改动后：空正文行一律成为上一行的 `end`（找不到收尾目标就丢弃），解析器不再产出
空正文行；上述 420 处归零，28436 行拿到真实 `end`。

### 其余差异

| 项 | 标准 | 本库 | 语料实测 |
| --- | --- | --- | --- |
| 分位数 | 1-3 位 | 1-4 位（更宽松） | 无 4 位 |
| 毫秒 4-6 位 | 字面毫秒（`450000` = 450000ms） | 按秒的小数部分截断（=450ms） | 0 例 |
| 标签后只剩空白 | 未定义 | 算正文（A5a 往返需要） | 0 例 |
| `[行标签][首字标签]文本` | 按重复行读（"局限性"一节） | 标签非递减时按"首字延迟"读成一行 | 199 行 / 20 文件，其中 197 行两标签相差 0.1-1.3 秒 |
| 隐式行尾 | 默认语义（持续到下一行开始） | 默认 `end=None`（未知），可选填充 | — |
| 空正文行 | 不存在这种行 | 整行只有尖括号标签时保留为空行（承载空字幕 cue） | 0 例 |

"毫秒 4-6 位"这一条标准内部本身有歧义：既然"不足 3 位视为在后位省略 0"（`1` → 100ms），
...
- [SPL Specification](https://moriafly.com/standards/spl.html)
  —— Salt Player Lyrics 语法标准（制定 2024-12-16，修订 2026-09-19）；本库的
  LRC 语义参照物，对照结果见上面的「SPL 对照」一节。
